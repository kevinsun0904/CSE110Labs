    ...
    <div>
      <div className="app-container">
        ...
        <div className="notes-grid">
          {notes.map((note) => {
            return (
              <div key={note.id} className="note-item" onClick={() => onNoteSelect(note)} style={{ background: currentTheme.background, color: currentTheme.foreground }}>
                <div className="notes-header">
                  <button onClick={() => onHeartClick(note.title)}>{likeMap[note.title] ? "❤️" : "🤍"}</button>
                  <button style={{ background: currentTheme.foreground, color: currentTheme.background }} onClick={() => onNoteDelete(note)} >x</button>
                </div>
